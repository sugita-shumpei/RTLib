#ifndef TEST_TESTLIB_PIPELINE_GROUP__H
#define TEST_TESTLIB_PIPELINE_GROUP__H
...
namespace TestLib
{
	...
	{
		PipelineGroup(const TestLib::Context* context, const OptixPipelineCompileOptions& options);
		~PipelineGroup();

		PipelineGroup(const PipelineGroup&) = delete;
		PipelineGroup& operator=(const PipelineGroup&) = delete;

		auto get_context() const noexcept -> const TestLib::Context*;
		...
		bool load_builtin_is_module(std::string name, const ModuleOptions& options, const OptixBuiltinISOptions& builtinIsOptions);

		auto get_module(std::string name) const noexcept -> Module*;
		bool has_module(std::string name) const noexcept;

		bool load_program_group_rg(
			std::string pgName, 
			std::string moduleName, 
			std::string entryName
		);
		bool load_program_group_ms(
			std::string pgName, 
			std::string moduleName, 
			std::string entryName
		);
		bool load_program_group_hg(
			std::string pgName, 
			std::string moduleNameCh, std::string entryNameCh, 
			std::string moduleNameAh, std::string entryNameAh, 
			std::string moduleNameIs, std::string entryNameIs
		);
		bool load_program_group_dc(
			std::string pgName, 
			std::string moduleName, 
			std::string entryName
		);
		bool load_program_group_cc(
			std::string pgName, 
			std::string moduleName, 
			std::string entryName
		);
	...
	private:
		const TestLib::Context* m_Context;
