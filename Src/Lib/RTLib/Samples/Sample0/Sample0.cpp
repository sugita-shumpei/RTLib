...
int main(int argc, const char** argv)
{
	...
	glfwInit();
	{
		...
		auto viewMatrix = glm::scale(
			glm::mat4(1.0f), glm::vec3(1.0f / glm::length(glm::vec3(camera->mLookAt[0], camera->mLookAt[1], camera->mLookAt[2])))
		) * glm::lookAt(
		...
		auto viewMatrixBase = modelMatrixMap.at(cameraNode).Inverse().Transpose();
		viewMatrix = viewMatrix * std::_Bit_cast<RTLib::Matrix4x4>(viewMatrixBase);
		...
		auto projMatrix = glm::perspective(fovy, camera->mAspect, 0.01f*camera->mClipPlaneNear, camera->mClipPlaneFar);
		...
		//			auto viewVertex = viewMatrix * RTLib::Vector4(vertex, 1.0f);
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			{
				gl->UniformMatrix4fv(modelPos, 1, GL_TRUE, &model[0][0]);
			...
			glfwPollEvents();
			{
				...
				}
				{
					viewMatrix = glm::scale(
						...
						glm::vec3(camera->mLookAt[0], camera->mLookAt[1], camera->mLookAt[2]),
						glm::vec3(camera->mUp[0], camera->mUp[1], camera->mUp[2])
					) * std::_Bit_cast<RTLib::Matrix4x4>(viewMatrixBase);
