		...
		namespace experimental
		{
			struct FbxBinaryAttributeView {
				...
				static auto ReadFromFile(std::ifstream& fbxFile)noexcept->FbxBinaryAttributeView
				{
					...
					if (IsValueTypeCode(attrib.type)) {
						switch (attrib.type) {
						case RTLib::Core::experimental::FbxBinaryAttributeView::TypeCode::eBool:
					...
					if (IsSpecialTypeCode(attrib.type)) {
				...
				std::variant<std::monostate, TypeArrayData, TypeSpecialData> data = std::monostate();
			...
			struct FbxBinaryNodeView
			{
				...
				struct     HeaderData {
					...
					auto GetEndPosInBytes()const noexcept -> uint32_t {
						if (type == HeaderType::eV1) { return data.v1.endPosInBytes; }
						else { return data.v2.endPosInBytes; }
					}
					auto GetNumAttributes()const noexcept -> uint32_t {
						if (type == HeaderType::eV1) { return data.v1.numAttributes; }
						else { return data.v2.numAttributes; }
					}
					auto GetSumAttributesAsBytes()const noexcept -> uint32_t {
						if (type == HeaderType::eV1) { return data.v1.sumAttributesAsBytes; }
						else { return data.v2.sumAttributesAsBytes; }
					}
					auto GetLenNodeName()const noexcept -> uint8_t {
						if (type == HeaderType::eV1) { return data.v1.lenNodeName; }
						else { return data.v2.lenNodeName; }
					}
...
int main()
{
	...
	}
	{
		...
		auto fbxHeaderVersionView = binaryNodeSet.at("Root/FBXHeaderExtension/FBXHeaderVersion");
		if (fbxHeaderVersionView->attributes[0].type==RTLib::Core::experimental::FbxBinaryAttributeView::TypeCode::eBool) {
			auto pData = binary.rawData.get() + fbxHeaderVersionView->attributes[0].begPosInBytes + 1;
			char v  = 0;
			std::memcpy(&v, pData, sizeof(v));
			std::cout << "FbxHeaderVersion: Bool  = " << v << " ;\n";
		}
		if (fbxHeaderVersionView->attributes[0].type == RTLib::Core::experimental::FbxBinaryAttributeView::TypeCode::eInt16) {
			auto pData = binary.rawData.get() + fbxHeaderVersionView->attributes[0].begPosInBytes + 1;
			int16_t v = 0;
			std::memcpy(&v, pData, sizeof(v));
			std::cout << "FbxHeaderVersion: Int16 = " << v << " ;\n";

		}
		if (fbxHeaderVersionView->attributes[0].type == RTLib::Core::experimental::FbxBinaryAttributeView::TypeCode::eInt32) {
			auto pData = binary.rawData.get() + fbxHeaderVersionView->attributes[0].begPosInBytes + 1;
			int32_t v = 0;
			std::memcpy(&v, pData, sizeof(v));
			std::cout << "FbxHeaderVersion: Int32 = " << v << " ;\n";
		}
		if (fbxHeaderVersionView->attributes[0].type == RTLib::Core::experimental::FbxBinaryAttributeView::TypeCode::eInt64) {
			auto pData = binary.rawData.get() + fbxHeaderVersionView->attributes[0].begPosInBytes + 1;
			int64_t v = 0;
			std::memcpy(&v, pData, sizeof(v));
			std::cout << "FbxHeaderVersion: Int64 = " << v << " ;\n";
