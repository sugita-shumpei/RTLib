...
void RTLibExtOPX7TestApplication::InitDefTracer()
{
    ...
    auto instanceGeometryDescs = rtlib::test::EnumerateGeometriesFromGASInstances(m_ShaderTableLayout.get(), m_SceneData.world);
    for (auto& [instanceName,geometryName] : instanceGeometryDescs)
    {
        if (m_SceneData.world.geometryObjModels.count(geometryName) > 0)
        {
            auto& geometry = m_SceneData.world.geometryObjModels[geometryName];
