...
extern "C" __global__ void     __raygen__default () {
    ...
    rtlib::test::TraceVertex vertices[RTLIB_EXT_OPX7_TEST_PATH_GUIDING_MAX_VERTEX_COUNT] = {};
    ...
    while (true) {
        ...
        if ((hrec.flags & PARAM_FLAG_USE_TREE) && !(params.flags & PARAM_FLAG_FINAL) 
            && (hrec.userData.depth < RTLIB_EXT_OPX7_TEST_PATH_GUIDING_MAX_VERTEX_COUNT)) {
            vertices[hrec.userData.depth].rayOrigin = hrec.rayOrigin;
            vertices[hrec.userData.depth].rayDirection = hrec.rayDirection;
            vertices[hrec.userData.depth].dTree = hrec.userData.dTree;
            vertices[hrec.userData.depth].dTreeVoxelSize = hrec.userData.dTreeVoxelSize;
            vertices[hrec.userData.depth].throughPut = hrec.userData.throughPut;
            vertices[hrec.userData.depth].bsdfVal = hrec.userData.bsdfVal;
            vertices[hrec.userData.depth].radiance =hrec.userData.radiance;
            vertices[hrec.userData.depth].woPdf = hrec.userData.woPdf;
            vertices[hrec.userData.depth].bsdfPdf = hrec.userData.bsdfPdf;
            vertices[hrec.userData.depth].dTreePdf = hrec.userData.dTreePdf;
            vertices[hrec.userData.depth].cosine = hrec.cosine;
            vertices[hrec.userData.depth].isDelta = (hrec.flags & HIT_RECORD_FLAG_DELTA_MATERIAL);

            printf("dTree=%p\n", hrec.userData.dTree);
            //if (hrec.userData.radiance.x > 0.0f) {
            //    printf("%lf %lf %lf\n", hrec.userData.radiance.x, hrec.userData.radiance.y, hrec.userData.radiance.z);
            //}
            for (int j = 0; j < hrec.userData.depth; ++j) {
                vertices[j].Record(hrec.userData.radiance);
            }
        }
        ...
        if (isnan(hrec.rayDirection.x)|| isnan(hrec.rayDirection.y)|| isnan(hrec.rayDirection.z)||
            ...
            printf("error (%lf %lf %lf) (%lf %lf %lf)\n", 
                hrec.rayDirection.x, hrec.rayDirection.y, hrec.rayDirection.z,
                hrec.userData.radiance.x, hrec.userData.radiance.y, hrec.userData.radiance.z
            );
            break;
        }
    ...
    if (hrec.flags & PARAM_FLAG_USE_TREE) {
        if (!(hrec.flags & PARAM_FLAG_FINAL)) {
            for (int j = 0; j < RTLib::Ext::CUDA::Math::min(hrec.userData.depth, (unsigned int)RTLIB_EXT_OPX7_TEST_PATH_GUIDING_MAX_VERTEX_COUNT); ++j) {
                //if (vertices[j].dTree) {
                //    printf("%lf %lf %lf\n", vertices[j].radiance.x / vertices[j].woPdf, vertices[j].radiance.y / vertices[j].woPdf, vertices[j].radiance.z / vertices[j].woPdf);
                //}
                vertices[j].Commit<rtlib::test::SpatialFilter::SpatialFilterNearest, rtlib::test::DirectionalFilter::DirectionalFilterNearest>(params.tree, 1.0f);
            }
        }
    }
    result += color;
...
extern "C" __global__ void __closesthit__radiance() {
    ...
    if (params.flags & PARAM_FLAG_USE_TREE) {
        dTree = params.tree.GetDTreeWrapper(hrec->rayOrigin);
        if (!dTree) {
    ...
    do{
        if (hgData->type == HIT_GROUP_TYPE_PHONG) {
            ...
            if ((params.flags & (PARAM_FLAG_USE_TREE | PARAM_FLAG_BUILD)) == (PARAM_FLAG_USE_TREE | PARAM_FLAG_BUILD))
            {
                if (params.tree.fraction <= RTLib::Ext::CUDA::Math::random_float1(xor32)) {
                    direction = dTree->Sample(xor32);
                    cosine    = RTLib::Ext::CUDA::Math::dot(direction, fNormal);
                    if (isnan(direction.x) || isnan(direction.y) || isnan(direction.z)) {
                        printf("newDirection1 is nan: new Direction1 = (%f, %f, %f) normal = (%f, %f, %f)\n", direction.x, direction.y, direction.z, fNormal.x, fNormal.y, fNormal.z);
                    }
                    bsdfPdf   = select_prob * RTLib::Ext::CUDA::Math::max(cosine * RTLIB_M_INV_PI, 0.0f)+(1.0f-select_prob) * getValPhongPDF(direction, reflDir, shinness);
                }
            ...
            currThroughput = prevThroughput * ((woPdf > 0.0f) ? (bsdfVal * fabsf(cosine) / woPdf) : make_float3(0.0f));
