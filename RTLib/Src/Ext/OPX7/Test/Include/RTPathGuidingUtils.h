...
namespace rtlib{
	namespace test {
		...
		namespace utils {
			template<typename T>
			void DownloadBuffer(RTLib::Ext::CUDA::CUDAContext* context, RTLib::Ext::CUDA::CUDABuffer* buffer, std::vector<T>& data) {
				auto desc = RTLib::Ext::CUDA::CUDABufferMemoryCopy();
				desc.dstData = data.data();
				desc.size = data.size() * sizeof(data[0]);
				desc.srcOffset = 0;
				RTLIB_CORE_ASSERT_IF_FAILED(context->CopyBufferToMemory(buffer, { desc }));

			}
			template<typename T>
			void UploadBuffer(RTLib::Ext::CUDA::CUDAContext* context, RTLib::Ext::CUDA::CUDABuffer* buffer, const std::vector<T>& data) {
				auto desc = RTLib::Ext::CUDA::CUDAMemoryBufferCopy();
				desc.srcData = data.data();
				desc.size = data.size() * sizeof(data[0]);
				desc.dstOffset = 0;
				RTLIB_CORE_ASSERT_IF_FAILED(context->CopyMemoryToBuffer(buffer, { desc }));
			}
		}
		...
		public:
			RTSTreeWrapper(RTLib::Ext::CUDA::CUDAContext* context, const float3& aabbMin, const float3& aabbMax, unsigned int maxDTreeDepth = 20)noexcept :m_Context{ context }, m_CpuSTree{ aabbMin,aabbMax }, m_MaxDTreeDepth{ maxDTreeDepth }{}
			void Upload()noexcept {
				...
				m_GpuSTreeNodes = std::unique_ptr<RTLib::Ext::CUDA::CUDABuffer>(m_Context->CreateBuffer({ RTLib::Ext::CUDA::CUDAMemoryFlags::eDefault,sizeof(sTreeNodes[0]) * std::size(sTreeNodes)        ,nullptr }));
				m_GpuDTreeWrappers = std::unique_ptr<RTLib::Ext::CUDA::CUDABuffer>(m_Context->CreateBuffer({ RTLib::Ext::CUDA::CUDAMemoryFlags::eDefault,sizeof(dTreeWrappers[0]) * std::size(dTreeWrappers)     ,nullptr }));
				m_GpuDTreeNodesBuilding = std::unique_ptr<RTLib::Ext::CUDA::CUDABuffer>(m_Context->CreateBuffer({ RTLib::Ext::CUDA::CUDAMemoryFlags::eDefault,sizeof(dTreeNodesBuilding[0]) * std::size(dTreeNodesBuilding),nullptr }));
				m_GpuDTreeNodesSampling = std::unique_ptr<RTLib::Ext::CUDA::CUDABuffer>(m_Context->CreateBuffer({ RTLib::Ext::CUDA::CUDAMemoryFlags::eDefault,sizeof(dTreeNodesSampling[0]) * std::size(dTreeNodesSampling),nullptr }));
				...
				utils::UploadBuffer(m_Context, m_GpuSTreeNodes.get(), sTreeNodes);
				utils::UploadBuffer(m_Context, m_GpuDTreeWrappers.get(), dTreeWrappers);
				utils::UploadBuffer(m_Context, m_GpuDTreeNodesBuilding.get(), dTreeNodesBuilding);
			...
			void Download() noexcept {
				...
				std::vector<DTreeWrapper> dTreeWrappers(gpuDTreeCnt);
				std::vector<DTreeNode>    dTreeNodesBuilding(gpuDTreeNodeCntBuilding);
				utils::DownloadBuffer(m_Context, m_GpuDTreeWrappers.get(), dTreeWrappers);
				utils::DownloadBuffer(m_Context, m_GpuDTreeNodesBuilding.get(), dTreeNodesBuilding);
		...
		private:
			...
			RTSTree                                       m_CpuSTree;
			std::unique_ptr<RTLib::Ext::CUDA::CUDABuffer> m_GpuSTreeNodes = {};//����
			std::unique_ptr<RTLib::Ext::CUDA::CUDABuffer> m_GpuDTreeWrappers = {};//���L
			std::unique_ptr<RTLib::Ext::CUDA::CUDABuffer> m_GpuDTreeNodesBuilding = {};//������
			std::unique_ptr<RTLib::Ext::CUDA::CUDABuffer> m_GpuDTreeNodesSampling = {};//������
		...
		public:
			RTSTreeWrapper2(RTLib::Ext::CUDA::CUDAContext* context, const float3& aabbMin, const float3& aabbMax, unsigned int maxDTreeDepth = 20)noexcept :m_Context{ context }, m_CpuSTree{ aabbMin, aabbMax }, m_MaxDTreeDepth{ maxDTreeDepth }{}
			void Upload()noexcept {
				//Uploadは両方必要
				const size_t gpuSTreeNodeCnt = m_CpuSTree.GetNumNodes();
				size_t gpuDTreeCnt = 0;
			...
			auto GetGpuHandle()const noexcept -> STree2 {
				...
				sTree.aabbMax = m_CpuSTree.GetAabbMax();
				sTree.aabbMin = m_CpuSTree.GetAabbMin();
				sTree.nodes = RTLib::Ext::CUDA::CUDANatives::GetGpuAddress<STreeNode2>(m_GpuSTreeNodes.get());
				sTree.fraction = 0.5f;
				return sTree;
			}
			void Reset(int iter, int samplePerPasses) {
