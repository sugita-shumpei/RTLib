#ifndef RTLIB_BACKENDS_CUDA_MATH_RANDOM_H
#define RTLIB_BACKENDS_CUDA_MATH_RANDOM_H
#if defined(__cplusplus) && !defined(__CUDA_ARCH__)
#include <type_traits>
#endif
#include <RTLib/Backends/CUDA/Math/TypeTraits.h>
    ...
    namespace Backends
    {
        namespace Cuda
        {
            namespace Math
            {
                ...
                {
                    const float2 rnd2 = RTLib::Ext::CUDA::Math::random_float2(rng);
                    const float a = RTLib::Ext::CUDA::Math::min(rnd2.x, rnd2.y);
                    const float b = RTLib::Ext::CUDA::Math::max(rnd2.x, rnd2.y);
