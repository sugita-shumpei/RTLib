#ifndef RTLIB_BACKENDS_CUDA_MATH_VECTOR_FUNCTION_H
#define RTLIB_BACKENDS_CUDA_MATH_VECTOR_FUNCTION_H
#include <cuda.h>
#include <RTLib/Backends/CUDA/Math/Preprocessors.h>
#include <RTLib/Backends/CUDA/Math/Math.h>
  ...
  namespace Backends
  {
    namespace Cuda
    {
      namespace Math
      {
        ...
        {
          return direction - 2.0f * RTLib::Backends::Cuda::Math::dot(direction, normal) * normal;
        }
        ...
        {
          return f0 + (1.0f - f0) * RTLib::Backends::Cuda::Math::pow5(1.0f - cosine_i);
        }
        RTLIB_INLINE RTLIB_HOST_DEVICE float fresnell(float cos_i, float ior)
        {
          const float cos_i_2 = cos_i * cos_i;
          const float sin_i_2 = RTLib::Backends::Cuda::Math::min(1.0f - cos_i_2, 1.0f);
        ...
        RTLIB_INLINE RTLIB_HOST_DEVICE float2 dir_to_canonical(const float3& d)
        {
            ...
            const float z = RTLib::Backends::Cuda::Math::clamp(d.z, -1.0f, 1.0f);
