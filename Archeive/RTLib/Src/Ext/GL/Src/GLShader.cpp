...
bool RTLib::Ext::GL::GLShader::ResetBinarySPV(const std::vector<uint32_t> &spirvData)
{
	...
	GLuint obj = GetResId();
	if (!obj || !m_SpvSupported || !m_PreAttachableState)
	{
		return false;
	}
...
bool RTLib::Ext::GL::GLShader::Specialize(const GLchar *pEntryPoint​, GLuint numSpecializationConstants​, const GLuint *pConstantIndex​, const GLuint *pConstantValue​)
{
	...
	if (!m_PreAttachableState->ownBinary)
	{
		return false;
	}
...
auto RTLib::Ext::GL::GLShader::GetShaderStage() const noexcept -> GLShaderStageFlagBits
{
...
GLuint RTLib::Ext::GL::GLShader::GetResId() const noexcept { return m_ResId; }
